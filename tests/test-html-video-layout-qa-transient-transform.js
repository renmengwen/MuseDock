...
function html({ overflow = 'visible', animation = 'self', overlap = false, viewport = false, baseOffset = 0 }) {
  ...
  const selfAnimation = animation === 'self' ? 'animation:safeRise 4s linear both' : animation === 'none' ? 'transform:translateY(40px)' : '';
  ...
  return `<!doctype html><html><head><style>
    ...
    @keyframes safeRise{0%{transform:translateY(40px)}20%,100%{transform:translateY(0)}}
    @keyframes constantRise{from{transform:translateY(40px)}to{transform:translateY(40px)}}
    @keyframes longRise{from{transform:translateY(40px)}to{transform:translateY(0)}}
...
(async () => {
  ...
  try {
    ...
    const outsideContainer = await inspect(
      ...
      html({ animation: 'outer', baseOffset: 40 }),
      11.95,
    );
    ...
    const unrelatedAnimation = await inspect(dir, 'visible-unrelated-animation', html({ baseOffset: 40 }));
