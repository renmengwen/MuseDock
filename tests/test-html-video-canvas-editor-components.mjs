...
assert.match(canvasEditing, /body, body \* \{\s*pointer-events: auto !important;/, '编辑器应恢复生成帧的点击命中');
assert.match(canvasEditing, /isEditableElement\(current\) && !isEffectivelyHidden\(current\)/, '点击候选应跳过透明 beat 及其后代');
...
assert.match(canvasUnit, /pendingJumpToEndRef/, '加载中点击跳到结尾不应丢失');
assert.match(canvasUnit, /iframeLoadedRef/, '必须区分真实 HTML onLoad 与空 iframe');
assert.match(canvasUnit, /if \(!iframeLoadedRef\.current \|\| !win\?\.document\?\.body\) return false;/, '空 iframe 不应被标记为已可编辑');
