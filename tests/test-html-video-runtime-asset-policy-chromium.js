...
(async () => {
  ...
  try {
    ...
    const shell = '<!doctype html><html><head><style>html,body{margin:0;width:640px;height:360px;background:#111}#root{position:absolute;inset:0;background-color:rgb(247,244,234)}[data-role="main-visual"]{position:absolute;left:0;top:0;width:640px;height:320px;z-index:1;background:linear-gradient(180deg,rgba(247,244,234,.4),rgba(247,244,234,0) 80px),linear-gradient(0deg,rgba(247,244,234,.3),rgba(247,244,234,0) 80px),rgb(247,244,234)}[data-test-overlay]{position:absolute;z-index:5;left:250px;top:150px;width:140px;height:60px;background:#fff;color:#000}</style><script>(()=>{const sheet=new CSSStyleSheet();const index=sheet.insertRule(".plain{color:#fff}");sheet.deleteRule(index)})()</script></head><body data-hv-canvas data-width="640" data-height="360"><div id="root" data-hv-canvas data-width="640" data-height="360"><div data-role="main-visual"></div><main><div data-test-overlay="overlay">模型文字 Overlay</div></main></div></body></html>';
        ...
        afterGoto: async ({ page, collector }) => {
          ...
          const stacking = await page.evaluate(() => {
            ...
            return {
              ...
              mainVisualBackground: getComputedStyle(document.querySelector('[data-role="main-visual"]')).backgroundColor,
              mainVisualBackgroundImage: getComputedStyle(document.querySelector('[data-role="main-visual"]')).backgroundImage,
    ...
    assert.equal(shotResult.stacking.mainVisualBackground, 'rgba(0, 0, 0, 0)', '嵌套主视觉平面的不透明底色必须被清除');
    assert.match(shotResult.stacking.mainVisualBackgroundImage, /linear-gradient/, '主视觉平面的可读性渐变必须保留');
    assert.equal(shotResult.stacking.overlayBackground, 'rgb(255, 255, 255)', '局部 Overlay 背景不得被透明化');
    ...
    for (const [name, attack] of [
      ...
      ['pre-guard-rule-method-restore', '<style id="pre-guard-rule-method">body{color:white}</style><script>const preGuardStyle=document.getElementById("pre-guard-rule-method").sheet.cssRules[0].style;preGuardStyle.setProperty("background-image","url(../assets/shot-a.png)");preGuardStyle.removeProperty("background-image")</script>'],
      ['transient-rule-custom-property-visual', '<style id="rule-custom-property">body{background-image:var(--rogue)}</style><script>window.__hvPlayAll=function(){setTimeout(function(){const style=document.getElementById("rule-custom-property").sheet.cssRules[0].style;style.setProperty("--rogue","url(../assets/shot-a.png)");setTimeout(function(){style.removeProperty("--rogue")},100)},20)}</script>'],
      ...
      ['transient-replace-sync-visual', '<script>const replaceSyncSheet=new CSSStyleSheet();replaceSyncSheet.replaceSync("body{color:white}");document.adoptedStyleSheets=document.adoptedStyleSheets.concat(replaceSyncSheet);window.__hvPlayAll=function(){setTimeout(function(){replaceSyncSheet.replaceSync("body{background-image:url(../assets/shot-a.png)}");replaceSyncSheet.replaceSync("body{color:white}")},20)}</script>'],
      ['transient-replace-visual', '<script>const replaceSheet=new CSSStyleSheet();replaceSheet.replaceSync("body{color:white}");document.adoptedStyleSheets=document.adoptedStyleSheets.concat(replaceSheet);window.__hvPlayAll=function(){setTimeout(function(){replaceSheet.replace("body{background-image:url(../assets/shot-a.png)}").then(function(){return replaceSheet.replace("body{color:white}")})},20)}</script>'],
    ...
    await fsp.writeFile(inlineStyleSource, shotHtml.replace('<main>', '<style id="web-idl-compatible">body{color:white}</style><script>window.__hvPlayAll=function(){const ruleStyle=document.getElementById("web-idl-compatible").sheet.cssRules[0].style;if(ruleStyle.constructor!==CSSStyleDeclaration)throw new Error("constructor changed");if(ruleStyle.setProperty!==CSSStyleDeclaration.prototype.setProperty)throw new Error("method identity changed");CSSStyleDeclaration.prototype.setProperty.call(ruleStyle,"color","red");const element=document.body;const transform=element.style.transform;const opacity=element.style.opacity;element.style.transform="translateX(10px)";element.style.opacity=".5";element.style.transform=transform;element.style.opacity=opacity}</script><main>'), 'utf8');
