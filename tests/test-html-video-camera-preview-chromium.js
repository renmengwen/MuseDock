...
function previewHtml(twoCues = false) {
...
(async () => {
  ...
  assert.ok(report.metrics.camera_samples.every(sample => Math.abs(sample.clock_time_sec - sample.sample_time_sec) < 1e-6), '浏览器实际共享时钟必须精确停在请求采样点');
  ...
  assert.deepEqual(jitter.metrics.camera_samples.map(sample => sample.sample_time_sec), [0.05, 0.15, 0.25, 0.35], '默认 Camera 路径必须从 cue 过渡窗派生密集采样');
