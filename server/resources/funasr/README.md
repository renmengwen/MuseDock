...
在 MuseDock 左侧栏点击 **抖音转写**，粘贴单条公开视频链接、分享文案或视频 ID。输入框下方可选择是否自动校订、是否抽帧，然后点击 **开始转写**。此工具复用 MuseDock 自己的抖音解析、媒体下载与 FFmpeg 能力，独立保存转写任务，不创建视频创作任务。

- **仅转写**：使用 FunASR，保存原始文字、句级 SRT、JSON 和音频。
- **自动校订**：转写完成后调用设置中心选择的分析模型，只校订文字与标点，原始文件保持不变。默认关闭，启用后会使用分析模型的配额。
- **按数量抽帧**：默认关闭；开启后输入 1–100 张。转写及可选校订完成后读取本地视频的真实时长，以“总时长 ÷ 数量”为间隔，按 0、间隔、2 × 间隔……截取 JPG。例如 60 秒抽 6 张，对应 0、10、20、30、40、50 秒；抽 1 张时只取 0 秒。结果页提供截图预览和本地打开入口。图片与记录时间点、SHA-256 的 `frames.json` 保存于任务的 `media/<视频ID>/frames/` 目录；抽帧失败保留文字、字幕和已保存的截图。
- **失败恢复**：校订失败仍可阅读、打开原始结果；“仅重试校订”复用已有转写，不再次下载或请求 ASR。
- **后台运行**：关闭弹框不会中断任务。重新打开或刷新页面可恢复上次任务的状态。服务进程重启后会标记中断，保留已生成产物。
- **删除历史**：点击记录右上角的删除按钮，确认后永久清理该记录的完整任务目录，包含视频、音频、原始转写、全部校订版本与中间文件。正在转写或校订时不能删除；其他历史可单独删除。取消保留主弹框和当前选择，删除失败可关闭占用文件后重试。
...
任务写入应用数据根目录的 `data/transcriptions/<任务ID>/`；Electron 使用自身的用户数据目录。文件包括原始 TXT/SRT/JSON、来源元数据、媒体 SHA-256、音频与每段 ASR 响应。校订版和校订记录保存在独立的 `corrections/<尝试ID>/` 中，记录每项原文、改文、理由与未变的时间码。“打开 TXT”“打开 SRT”和“打开校订记录”会用系统默认应用打开对应本地文件，也可点击“打开所在文件夹”；打开前会检查文件 SHA-256。打开操作需在运行 MuseDock 的电脑上执行。
