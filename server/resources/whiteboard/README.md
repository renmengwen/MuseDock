...
- 三种画幅的字幕都按逗号、句号等自然停顿逐句切换，每次只显示一条单行短句，过长分句按画布宽度继续拆短。“第三，”“最后，”等很短的引导语跟随下一分句显示。有旁白时保留同次语音的原生字级时间；无旁白与输入 SRT 在原字幕时间范围内分配短句时长，不改变分镜边界。
- 图片按供应商支持的尺寸生成，再等比适配输出画布、补齐纸色，不拉伸或裁切文字；合成拒绝混用不同画幅的单幕。

已生成的 MP4 和已确认产物不会自动改写。旧任务使用新版字幕时，需要重新确认制作设置后生成新版本；正文、声音及时间来源保持一致时，已有有效音频和同次原生字幕会复用，字幕更新在本地完成。

“制作设置”支持选择字幕颜色和 24–96 px 字号，并即时查看样式示例。默认白字黑描边；字号留空时横屏使用 48 px、竖屏使用 52 px。字号越大，每条字幕容纳的文字越少，仍保持单行。新建任务和已有任务都可设置，关闭字幕时保留已选样式。只改颜色或字号会复用仍有效的旁白、线稿、落墨编排和单幕视频，在最终合成时更新字幕；成片区打开的 SRT 与本次成片分段一致。
