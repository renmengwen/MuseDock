...
6. 在产物区试听、查看图像、播放单幕和最终视频，点击“打开”用系统默认应用打开已保存的 MP4、WAV、SRT 或标注 JSON，也可打开文件所在文件夹。

设置改变后重新确认方案。只改字幕、画笔等设置时，服务端按输入身份重验并复用仍有效的上游产物，避免重复生成语音、图片或编排。修改指定幕时，仅使该幕的相关下游失效。

## 背景音乐

“制作设置 → 背景音乐”支持“不使用 BGM”和“使用 BGM”，默认关闭。开启后，最终成片混入内置轻钢琴曲 **First Light Particles**（Yoiyami，CC0），旁白试听和本地保存的完整 WAV 仍保留纯旁白。豆包与 MiniMax 共用本地混音流程，不向供应商额外请求音乐；主题、正文和 SRT 都可以只配背景音乐、不生成旁白。
...
- 横屏 `4:3`：1440×1080，适合人物关系、词组与箭头组成的手写图解；也可用于原有模板。
- 画幅通过 `input.aspectRatio` 在创建任务时确定，随方案及媒体身份保存；需要其他画幅时开启新创作，不在原任务中途切换。
- 竖屏与 4:3 字幕按画布宽度分段，仍保留同次语音的原生字级时间；字幕最多两行。图片按供应商支持的尺寸生成，再等比适配输出画布、补齐纸色，不拉伸或裁切文字；合成拒绝混用不同画幅的单幕。

“白底手写图解”使用纯白纸面、黑色人物线条、红蓝手写关键词与少量橙色结论，推荐选择横屏 4:3，也支持其余两种画幅。选择模板不会改变已选画幅、时长、旁白、BGM、画笔或字幕设置，制作与确认流程保持原样。

每幕方案单独列出“画内原文”，在线稿预览中按清单核对错字、漏字和多余文字；自动推进时在原有视觉检查中完成核对，有问题时按原流程等待处理。文字直接保留生成图片中的手写字形，不强制替换为后期字体。按标题、人物、完整词组、气泡、关系线及结论的叙事顺序逐区绘制，支持最多 24 个区域和多边形边界；短幕需合并区域，每区至少 0.1 秒，末尾保留半秒。第一版沿图像笔迹渐显，不代表真实汉字笔顺。
...
主题、正文和 SRT 都可在“制作设置 → 旁白方式”选择“不使用旁白”，无需配置 TTS。主题与正文复用已确认方案按目标总时长和文本长度分配的字幕、分镜时间，正式产物标记为 `planned`；SRT 继续保留 `source_srt` 时间轴。无旁白流程不生成音频、不请求 ASR，仍生成保存在本地的 SRT，供字幕显示、落墨编排与单幕渲染共用。
...
- 首帧为模板对应的干净纸底，末尾至少停留 0.5 秒。

六个原有模板沿用原绘制核心与暖米黄纸色。“白底手写图解”通过独立适配器使用冻结的纯白和原色落墨参数，矩形或多边形区域共用覆盖检查与绘制边界，不把红蓝文字临时画成黑色。

上游文件、来源 SHA 和本地文件 SHA 记录在 `sources.json`。画笔素材原样保留 `@moveR` 标识。运行时只使用本仓库代码和应用数据目录，不读取 Codex Skill 配置或依赖其环境。

## 状态、审阅与恢复

媒体阶段拥有冻结的阶段 schema、独立 attempt、文件 SHA、产物身份和批准。初始草案、模型候选、技术通过与批准分别保存。文件发布与任务修改共用队列，旧请求不能重建已删除任务。媒体文件接口只接受已登记的 artifact ID，重新检查路径、大小与 SHA 后提供播放或打开本地文件；打开操作需在运行 MuseDock 的电脑上执行。
...
落墨分镜即使没有生成预览，也可打开详情，查看本幕历次请求的状态、起止时间、耗时、候选保存情况与处理建议。分镜中的视觉诊断保留固定类别、HTTP 状态和是否收到响应；任务详情的“API 返回记录”另行提供每次成功或失败请求的脱敏返回正文，可查看详情并复制反馈。内容方案首次生成和自动补正分别记录原始模型返回及校验原因。旧版本未保存的返回无法补录。达到输出上限或带有未完成标记的响应即使包含可解析 JSON，也保持结果待核实，不发布为完整候选。查看详情不触发恢复、授权或新的模型请求。
...
合并时使用每幕帧数派生精确 concat 时长，避免 MP4 容器毫秒取整累积误差。最终产物检查 H.264、所选画幅（1920×1080、1080×1920 或 1440×1080）、60 fps、yuv420p、累计帧数、音画时长、AAC/24 kHz/单声道以及完整解码，不用补帧或 `-shortest` 掩盖时钟错误。
...
node tests/test-whiteboard-handwritten.js
...
data/runtime/whiteboard/Scripts/python.exe -X utf8 tests/test-whiteboard-handwritten-render.py
...
这些自动验证使用隔离存储与模型替身，真实执行绘制、ffmpeg、播放器和文件访问。`scripts/debug/whiteboard-live-verify.cjs --live` 是独立的真实调用验收入口，可能计费，不属于 `npm test`；结果和去敏请求记录保存在 `.codex-runtime/whiteboard-live-verification/`，真实 provider 证据与本地 fixture 分开报告。
