...
function buildFrameHtmlPrompt({
  ...
  return [
    ...
    '- 编写 HTML 前先在内部选择一种主布局：左右分栏、上下分区、全出血主视觉 + 独立文字面板、或单一结构图；只选一种，不要把多套布局叠在同一帧。',
    '- 先划分背景层、主视觉区、文字信息区和底部 140px 字幕安全区。背景图片可以全出血，但所有标题、正文、卡片、图表标签、进度条和装饰性 overlay 都必须停留在字幕安全区上方。',
    '- 标题、正文、列表和卡片正文必须放在同一个明确的文字容器内，并在容器内部使用 grid/flex 正常流、gap 和 padding 排版；不要分别用 position:absolute 把多段文字散落叠放。',
    '- 文字卡片使用 min-height 或由内容自然撑开，不要用固定 height 裁切文字；内容放不下时优先减少装饰、缩短画面文案或调整分栏比例，不要压缩行高或把元素叠在一起。',
    '- position:absolute 仅用于背景、主视觉区域、单个文字面板或局部装饰；同一区域内只能有一个主要内容容器，装饰层和伪元素必须位于可读文字下方。',
    '- 入场和强调动画只改变 transform 与 opacity，不动画修改 width、height、top、left、right、bottom、padding 或 font-size，避免动画过程中重新排版产生瞬时遮挡。',
    '- 首帧必须立即显示背景或主视觉，禁止让根画布、主舞台或全部主要内容同时 opacity:0；动画完成后主要内容保持稳定可见直到本帧结束。',
    '- overflow:hidden 只用于根画布和明确的图片裁切容器；文字容器不得用固定高度配合 overflow:hidden 掩盖溢出。',
