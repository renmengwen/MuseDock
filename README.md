...
点击左侧栏的 **抖音转写**，在弹框中粘贴公开视频链接或分享文案，选择是否自动校订后开始。工具使用独立的 FunASR 服务提取完整文字和句级字幕；启用自动校订时调用已配置的分析模型，分别保留原始版、校订版与修改记录，可下载 TXT/SRT。

在 **设置 → 模型配置 → 全局模型选择 → ASR 转写** 中可直接选择 **FunASR（本地，默认）**，无需添加供应商或填写 API Key。默认连接 `http://127.0.0.1:8000/v1` 的 Paraformer 服务，地址可在下拉框下方修改。已有 FunASR 环境可直接接入，新用户可使用项目附带的独立服务脚本；安装、启动与使用步骤见[抖音转写与 FunASR 说明](./server/resources/funasr/README.md)。
