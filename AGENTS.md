...
- 所有开发工作默认只能在 `dev` 分支进行，包括代码修改、文档修改、依赖调整、配置变更、格式化和提交。
- 在修改任何文件前，必须先确认当前 Git 分支。
- 如果当前分支是 `main`，必须立即停止工作，不得编辑文件、生成文件、格式化、提交或推送，并提醒用户先切换到 `dev` 分支。
- `main` 分支只用于稳定版本、发布或从 `dev` 合并，不直接承接日常开发。
- 如果用户明确要求在 `main` 分支操作，需要先复述风险并等待用户再次确认。
