...
async function main(){
  ...
  try {
    ...
    assert.equal(await page.getByRole('tab',{name:'输入与设置',exact:true}).count(),0,'任务页不应保留前置输入页签');
    await page.getByRole('button',{name:'本地任务参数',exact:true}).click();
    const parameters=page.getByRole('dialog',{name:'本地任务参数',exact:true});
    assert.equal(await parameters.getByRole('tab',{name:'输入与参考',exact:true}).count(),0,'创建后不再编辑原始输入和参考文本');
    assert.equal(await parameters.getByLabel('修改创作输入',{exact:true}).count(),0);
    await page.keyboard.press('Escape');await parameters.waitFor({state:'hidden'});
    ...
    await page.getByRole('button',{name:'本地任务参数',exact:true}).click();
    const settings=page.getByRole('dialog',{name:'本地任务参数',exact:true});await settings.waitFor();
    ...
    const titleBox=await settings.getByRole('heading',{name:'本地任务参数',exact:true}).boundingBox();
    ...
    await page.getByRole('button',{name:'本地任务参数',exact:true}).focus();await page.keyboard.press('Enter');
    await settings.waitFor();await page.keyboard.press('Escape');await settings.waitFor({state:'hidden'});
    await page.waitForFunction(() => document.activeElement?.textContent?.trim() === '本地任务参数', { timeout: 3000 });
    assert(await page.getByRole('button',{name:'本地任务参数',exact:true}).evaluate(element=>element===document.activeElement),'关闭弹窗应回到触发按钮');
    assert.deepEqual(errors,[],'浏览器不应出现运行时错误');
    assert.deepEqual(network,[],'正常流程不应产生失败请求');
    await page.getByRole('button',{name:'本地任务参数',exact:true}).click();await settings.waitFor();
