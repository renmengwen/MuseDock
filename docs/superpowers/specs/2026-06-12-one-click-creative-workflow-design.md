...
- 保留现有内容抓取、抓取记录、素材工作台、高级成片和设置页面；仅将项目默认入口调整为新的一键创作页面。
...
项目默认路由应调整为新的一键创作页面。现有页面全部保留，并继续通过导航访问：

- 内容抓取页：保留现有平台抓取能力。
- 抓取记录页：保留历史记录、评论和媒体入口。
- 素材工作台：保留现有媒体准备、ASR、帧素材查看和本地文件打开能力。
- 高级成片 / HyperFrames Studio：保留现有高级成片编辑、工程检查、文件编辑、渲染和巡检能力。
- 设置页：保留模型、ASR、TTS 和运行配置。

新的一键创作页是首屏入口，不删除或弱化原有专业工作台。它负责把常用链路串起来；当用户需要细调、排障或高级编辑时，可以跳转到原有素材工作台或高级成片页面继续处理。
