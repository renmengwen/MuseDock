...
import { AudioLines, Check, Copy, Download, LoaderCircle, RotateCw } from 'lucide-react';
...
export function TranscriptionTool({ compact = false }) {
  ...
  const busy = submitting || isRunning(job);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    const savedId = lastTaskId();
    Promise.allSettled([api.getTranscriptionCapabilities(), savedId ? api.getTranscription(savedId) : Promise.resolve(null)])
      .then(([config, previous]) => {
        if (cancelled) return;
        if (config.status === 'fulfilled') setCapabilities(config.value.data);
        else setError(config.reason.message || '读取转写配置失败，请重新打开工具。');
        if (previous.status === 'fulfilled' && previous.value?.data) {
          setJob(previous.value.data);
          if (!job) {
            setSource(current => current || previous.value.data.source?.url || '');
            setAutoCorrect(previous.value.data.autoCorrect === true);
          }
        }
        else if (previous.status === 'rejected' && previous.reason.status !== 404) setError(previous.reason.message);
      }).finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [open]);

  useEffect(() => {
    if (!open || !isRunning(job)) return undefined;
    let cancelled = false;
    let timer;
    let failures = 0;
    setPollError('');
    const poll = async () => {
      try {
        const response = await api.getTranscription(job.id);
        if (cancelled) return;
        failures = 0;
        setPollError('');
        setJob(response.data);
        if (isRunning(response.data)) timer = window.setTimeout(poll, 1500);
      } catch (cause) {
        if (cancelled) return;
        failures += 1;
        setPollError(`${cause.message} 任务状态暂时无法更新，可点击“刷新状态”。`);
        if (failures < 3) timer = window.setTimeout(poll, 3000);
      }
    };
    timer = window.setTimeout(poll, 500);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [open, job?.id, job?.status, pollVersion]);
  ...
  function rememberJob(nextJob) {
    setJob(nextJob);
    try { localStorage.setItem(LAST_TASK_KEY, nextJob.id); } catch {}
  }
  ...
  async function start(event) {
    ...
    setSubmitting(true);
    setError(''); setNotice(''); setPollError('');
    try {
  ...
  async function copyText() {
    try {
      ...
      setNotice('文本已复制。');
    } catch { setError('浏览器不允许自动复制，请选中文本复制或下载 TXT。'); }
  }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size={compact ? 'icon' : 'default'}
          className={compact ? 'size-[34px] rounded-lg text-[#4b5563]' : 'h-11 w-full justify-start gap-2 px-2.5 text-[#30343b]'}
          aria-label="打开抖音转写工具" title="抖音转写">
          <AudioLines size={16} />
          {!compact && <span>抖音转写</span>}
        </Button>
      </DialogTrigger>
      <DialogContent className="w-[calc(100vw-2rem)] max-h-[calc(100dvh-2rem)] overflow-y-auto transition-none sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><AudioLines className="size-5" />抖音转写</DialogTitle>
          <DialogDescription>粘贴视频链接，提取完整文字与字幕。可选择由分析模型自动校订。</DialogDescription>
        </DialogHeader>
        ...
        <form className="grid gap-3" onSubmit={start}>
          ...
          {loading ? <p className="m-0 flex items-center gap-2 text-sm text-muted-foreground" role="status"><LoaderCircle className="size-4 animate-spin" />正在读取工具配置和上次任务...</p> : (
          ...
          <Button type="submit" disabled={busy || loading || !!loginAction || !source.trim() || !capabilities?.asrReady || (autoCorrect && !capabilities?.textReady)}>
            {busy && !pollError ? <LoaderCircle className="size-4 animate-spin" /> : <AudioLines className="size-4" />}
            {submitting ? '正在提交...' : pollError && isRunning(job) ? '等待更新任务状态' : isRunning(job) ? '正在转写处理...' : '开始转写'}
          </Button>
        </form>

        {error && <p role="alert" className="m-0 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</p>}

        {job && <section className="grid gap-2 rounded-lg border p-3" aria-label="转写进度">
          {job.source?.title && <p className="m-0 line-clamp-2 text-sm font-medium">{job.source.title}</p>}
          <div className="flex items-start gap-2 text-sm" role="status" aria-live="polite">
            {isRunning(job) && !pollError ? <LoaderCircle className="mt-0.5 size-4 shrink-0 animate-spin" /> : job.status === 'succeeded' ? <Check className="mt-0.5 size-4 shrink-0 text-emerald-600" /> : null}
            <span>{pollError || job.message}</span>
          </div>
          ...
          {isRunning(job) && <p className="m-0 text-xs text-muted-foreground">关闭弹框后任务继续运行，可重新打开查看。</p>}
          {pollError && <Button variant="outline" size="sm" onClick={() => setPollVersion(value => value + 1)}><RotateCw />刷新状态</Button>}
        ...
        {job?.result && <section className="grid gap-3" aria-label="转写结果">
          ...
          <Tabs value={version} onValueChange={setVersion}>
            ...
            <TabsContent value="raw"><pre className="m-0 max-h-64 overflow-auto whitespace-pre-wrap break-words rounded-lg border bg-muted/20 p-3 font-sans text-sm leading-7">{job.result.rawText}</pre></TabsContent>
            <TabsContent value="corrected"><pre className="m-0 max-h-64 overflow-auto whitespace-pre-wrap break-words rounded-lg border bg-muted/20 p-3 font-sans text-sm leading-7">{job.result.correctedText}</pre></TabsContent>
          </Tabs>
